...
int (vg_draw_pixel)(uint16_t x, uint16_t y, uint32_t color) {
  ...
  if (vmi_p.BitsPerPixel == 8) {
    ...
    uint32_t *video_ptr = (uint32_t *)video_mem;
...
int (clear_screen)() {
  if (memset(video_mem, 0, vmi_p.XResolution * vmi_p.YResolution * ((vmi_p.BitsPerPixel + 7) / 8)) == NULL) {
    return 1;
  }
